...
unsigned long timer = 1000/NUMELEMENTS(ringMatrix)/2; // tuned to half a second, max brightness
int brightness = 1; // percentage


void loop() {
  for (uint16_t rowCnt = 0; rowCnt < NUMELEMENTS(ringMatrix); rowCnt++){
    // for (int waiter = 0; waiter < floor(600/ringMatrix[rowCnt].numElements); waiter++){
    while(millis() - pulse_time < timer){
      for (uint16_t colCnt = 0; colCnt < ringMatrix[rowCnt].numElements; colCnt++){
        LED_ON(ringMatrix[rowCnt].pins[colCnt],ringMatrix[rowCnt].numElements);
      }
    }
    pulse_time = millis();
  }
...
void LED_ON(int LED,float LED_amount){
    LED = LED - 1;

    float LED_time = timer/LED_amount; // total time


    pinMode(pins[LOW_POS[LED]], OUTPUT);
    pinMode(pins[HIGH_POS[LED]], OUTPUT);
    delayMicroseconds(LED_time*brightness); // time on
    
    digitalWrite(pins[LOW_POS[LED]],LOW);
    digitalWrite(pins[HIGH_POS[LED]],HIGH);
    delayMicroseconds(LED_time*(brightness)); // time o

    pinMode(pins[LOW_POS[LED]], INPUT);
    pinMode(pins[HIGH_POS[LED]], INPUT);    
    delayMicroseconds(LED_time*(1-brightness)); // time off
